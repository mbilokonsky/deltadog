...
// I'm in the process of porting these examples into the `examples/` folder, which exposes the deltastore via GraqhQL
...
const state2 = [
  DD.createNameRelationship(universe.gallery, "GalleryName2", {
    timestamp: t2,
    $debug: "name gallery again"
  })
];

const updatedGallery = DD.integrate(
  universe.gallery,
  all_deltas.concat(state2)
);
...
const flattenToMostRecentName = (parent, property, deltas, formatID) => {
  const most_recent = deltas.reduce(
    (acc, val) => {
...
const formatted_flattened_updated_gallery_with_flattened_name = DD.flatten(
  updatedGallery,
  rules_2
);
log(
  "gallery - formatted with custom name logic",
  formatted_flattened_updated_gallery_with_flattened_name
);
...
const get_money_reducer = parent => {
  return (running_total, delta) => {
    ...
    const money_destination_pointer =
      delta.pointers[guids.pointers.money_target];

    const money_source = money_source_pointer
      ? money_source_pointer.target
      : null;
    const money_destination = money_destination_pointer
      ? money_destination_pointer.target
      : null;
...
const formatted_flattened_updated_gallery_with_flattened_name_and_summed_money = DD.flatten(
  updatedGallery,
  rules_3
);
log(
  "gallery + money logic",
  formatted_flattened_updated_gallery_with_flattened_name_and_summed_money
);
...
const gallery_after_sale = DD.integrate(
  universe.gallery,
  all_deltas.concat(state2).concat(state3)
);
...
const post_sale_flattened_with_ownership_transfer = DD.flatten(
  gallery_after_sale,
  rules_4
);
log(
  "gallery - post-sale + ownership transfer",
  post_sale_flattened_with_ownership_transfer
);
...
const back_to_the_future = {
  ...
  collector1: DD.flatten(
    DD.integrate(universe.collector1, filtered_deltas),
    rules_4
  )
};
...
const get_transaction_flattener = (parent, formatID) => {
  return (acc, delta) => {
    ...
    const formatted_seller = old_owner
      ? formatID(old_owner.target)
      : "<nobody>";
...
const painting_post_flattener = painting => {
  ...
  painting.average_price = painting.transactions.reduce(
    (acc, val, index, collection) => {
